...
class ORd:
    ...
    def calc_INa(self):
        # INa current
        mss = 1.0 / (1.0 + np.exp(( - (self.v + 39.57)) / 9.871))
    ...
    def calc_INaL(self):
        ...
        thL = 200.0
        hLssp = 1.0 / (1.0 + np.exp((self.v + 93.81) / 7.488))
        thLp = 3.0 * thL
    ...
    def calc_ICaL_ICaNa_ICaK(self):
        self.dss = 1.0 / (1.0 + np.exp(( - (self.v + 3.940)) / 4.230))
    ...
    def calc_INaCaK(self):
        ...
        hca = np.exp((qca * self.v * self.F) / (self.R * self.T))
    ...
    def calc_ca_release_SR_intrac_space(self, ICaL):
        ...
        Jrel = ((1.0 - fJrelp) * self.Jrelnp + fJrelp * self.Jrelp)
        return Jrel
